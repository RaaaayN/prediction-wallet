...
| backend | 2026-03-25 — #11 confidence scoring + #1 policy-as-code | 2026-03-25 | Current |
| ui | 2026-03-25 — event_type badges + drift_before/slippage_pct columns | 2026-03-25 | Current |
| strategy | 2026-03-25 — #6 correlation + #5 stress testing + #3 dynamic sizing | 2026-03-25 | Current |
| usecases | 2026-03-25 — Évaluation 15 features (8 Accept, 7 Defer) | 2026-03-25 | Current |

### État de la roadmap — 8 features Accepted

| # | Feature | Code | Câblé dans le cycle agent |
|---|---------|------|--------------------------|
| #15 | Explainability (per-trade audit fields) | ✅ commit e384ed0 | ✅ `execute()` peuple 5 champs |
| #12 | Event semantics (event_type + tags) | ✅ commit e384ed0 | ✅ 5 call sites peuplés |
| #11 | Confidence scoring + data_freshness | ✅ commit cf34f6a | ✅ `decide()` injecte la fraîcheur |
| #8  | Vol/size-adjusted slippage | ✅ commit e384ed0 | ⚠️ params optionnels — non passés depuis l'agent |
| #6  | Sector concentration policy check | ✅ commit 373e966 | ✅ Layer 2 soft block dans PolicyEngine |
| #5  | Stress testing (4 scénarios de crise) | ✅ commit 373e966 | ⚠️ standalone — absent du PDF et de l'API |
| #1  | Policy-as-code (PolicyConfig + 3 layers) | ✅ commit cf34f6a | ✅ chargé depuis profil actif |
| #3  | Dynamic sizing (inverse-vol weights) | ✅ commit 373e966 | ⚠️ param optionnel — vol_blend absent des profils |

**Résultat : 5/8 features effectivement actives en cycle. 3 implémentées mais dormantes (#8, #5, #3).**

### Cross-Agent Dependencies

- **#3 → team-backend** : `volatilities` disponible dans strategies mais `portfolio_agent.py` ne passe pas les données de vol. `vol_blend` absent de `profiles/*.yaml`. Wire-up = tâche backend pure.
- **#8 → team-backend** : même situation — `volatility` param dans `apply_slippage` nécessite que le vol par ticker soit passé depuis `MarketService` à travers le cycle agent.
- **#5 → team-backend** : `run_stress_test` prêt dans `engine/backtest.py` mais non appelé depuis `ReportingService`. Wire-in PDF (section 7) + endpoint `/api/stress`.
- **kill switch threshold** : les 4 scénarios de stress déclenchent tous le kill switch à 10%. Recommandation : relever à 15–20% dans les profils growth/crypto si paper mode envisagé.

### Top 3 Priorities

1. **[team-backend]** — Câbler dynamic sizing (#3) end-to-end — Ajouter `vol_blend: 0.3` dans `profiles/*.yaml`, passer `volatilities` depuis `MarketService` vers `portfolio_agent.py` → `get_trades()`. Sans ce wire-up, #3 est implémenté mais entièrement dormant. Risque : `vol_blend=1.0` par défaut couperait les targets crypto de 20% → ~5%, régler à 0.3.

2. **[team-backend]** — Câbler stress testing (#5) dans le PDF + exposer `/api/stress` — `run_stress_test` retourne des résultats pour 4 scénarios de crise mais aucun utilisateur ne peut les voir. Wire dans `ReportingService.generate_pdf()` (section 7) + endpoint GET `/api/stress`.

3. **[décision utilisateur]** — Ouvrir la PR `feat/agent-team-skills → main` — Toutes les 8 features Accept sont committées (e384ed0, cf34f6a, 373e966), 196+ tests verts, aucun blocker. Le branch est prêt. Les wire-ups #3/#5 peuvent se faire sur main ou dans une PR de suivi.

### Identified Risks

- **#3 dormant** : `vol_blend` non configuré → dynamic sizing inactif en production. Ne pas activer sans tester l'impact sur les targets crypto.
- **Kill switch trop conservateur pour stress** : 10% déclenche sur tous les scénarios. Aucune discrimination entre scénarios. À configurer par profil si paper mode envisagé.
- **`MarketSnapshot.research_summary`** toujours dans `agents/models.py` (toujours `""`). Cosmétique, impact nul.

### Recommended Action Plan

1. **Lancer `/team-backend`** : *"wire volatilities + vol_blend into portfolio_agent.py and profiles/*.yaml (activates #3), then wire run_stress_test into ReportingService PDF section 7 and add GET /api/stress (surfaces #5)"*
2. **Optionnel — `/team-ui`** : onglet correlation heatmap (données `rolling_correlation` déjà dans market DB).
3. **Ouvrir la PR** `feat/agent-team-skills → main` une fois le wire-up commité, ou dès maintenant si préféré.

### Stale Reports (if any)
(aucun — tous les agents ont des sessions 2026-03-25)

---

## Lead Report: 2026-03-26 10:30
**Last Updated:** 2026-03-26 10:30

### Team Status
| Agent | Last Session | Last Updated | Status |
|-------|-------------|-------------|--------|
| backend | 2026-03-26 — vol_blend + stress PDF + /api/stress | 2026-03-26 | Current |
| ui | 2026-03-26 — Correlation heatmap tab + /api/correlation | 2026-03-26 | Current |
| strategy | 2026-03-25 — #6/#5/#3 implémentés | 2026-03-25 | Current |
| usecases | 2026-03-25 — 15 features évaluées (8 Accept, 7 Defer) | 2026-03-25 | Current |

### Roadmap finale — état de toutes les 8 features Accept

| # | Feature | Code | Câblé et actif |
|---|---------|------|----------------|
| #15 | Explainability | ✅ | ✅ |
| #12 | Event semantics | ✅ | ✅ |
| #11 | Confidence scoring | ✅ | ✅ |
| #8  | Vol-adjusted slippage | ✅ | ✅ vol passé depuis MarketService |
| #6  | Concentration risk policy | ✅ | ✅ |
| #5  | Stress testing | ✅ | ✅ PDF section 7 + /api/stress |
| #1  | Policy-as-code hiérarchique | ✅ | ✅ |
| #3  | Dynamic sizing (vol_blend=0.3) | ✅ | ✅ câblé dans portfolio_agent.py |

**8/8 features actives. Aucune feature dormante.**

### Cross-Agent Dependencies

- **backend → ui** : `/api/stress` est prêt (response shape : `[{scenario, pnl_dollars, pnl_pct, kill_switch_triggered, ...}]`). team-ui peut ajouter un onglet "Stress Test" sans aucun changement backend.
- **Aucune autre dépendance bloquante.**

### Top 3 Priorities

1. **[team-ui]** — Ajouter un onglet "Stress Test" dans `ui/index.html` — L'endpoint `/api/stress` retourne les 4 scénarios de crise (COVID 2020, GFC 2008, rate shock 2022, tech selloff) avec P&L, valeur post-scénario, et flag kill switch. Tableau simple + stat cards. Backend recommande explicitement ce wire-up UI.

2. **[décision utilisateur]** — Ouvrir / merger la PR `feat/agent-team-skills → main` — La branch contient toutes les 8 features Accept, toutes les corrections d'infrastructure (dead code, policy split, init-once DB, SSE cleanup), et plus de 196 tests verts. Il n'y a aucun item ouvert bloquant le merge.

3. **[team-usecases]** — Re-run pour valider l'état final — Depuis l'audit du 2026-03-25, 3 features ont été câblées en production (#3, #5, #8), un onglet Correlation a été ajouté, et `/api/stress` est exposé. Un audit frais peut identifier si des features nouvellement exposées ont une valeur réelle ou si elles ajoutent de la complexité inutile.

### Identified Risks

- **Kill switch à 10% déclenche sur tous les scénarios de stress** : aucune discrimination entre scénarios de gravité différente. Si paper mode envisagé, configurer des seuils par profil (`conservative` = 10%, `growth`/`crypto_heavy` = 15–20%).
- **`vol_blend=0.3`** : paramètre non encore exposé en tant que param API. Le backend recommande d'ajouter un override dans `/api/run/run-cycle` pour expérimentation sans modifier les profils YAML.
- **Deferred features** (#2 régimes, #4 risk parity, #9 multi-agent) : tous légitimes mais prématurés selon usecases. À ne pas commencer avant que le baseline actuel soit stable en production.

### Recommended Action Plan

**Immédiatement (dans l'ordre) :**
1. **Merger `feat/agent-team-skills → main`** — sprint terminé, aucun item bloquant.
2. **`/team-ui`** — onglet Stress Test (endpoint `/api/stress` prêt).

**Ensuite (nouvel élan) :**
3. **`/team-usecases`** — audit de l'état final avant de démarrer le prochain sprint.
4. Évaluer si les features Deferred (#2, #4, #9) sont maintenant prêtes à démarrer — usecases avait dit "après #11 + #15 stables". C'est le cas maintenant.
